...
3. **Hot-store message cache.** `Store.cache_message/3` for each user message (key = hash of reconstructed content) and once for the assistant message (key = `:assistant_message_hash`). No audit cast. Cache entry shape is `{:user_message, sanitized_text}` for user messages, matching the shape written by the pipeline's `reduce_with_cache` loop. **Optimization:** On Turn 2+, the pipeline already caches newly-sanitized messages (cache misses) during `reduce_with_cache`, so `persist_turn` skips the user-message cache loop when `is_new == false` to avoid redundant writes. The assistant-message cache write always runs (the pipeline doesn't cache assistant messages).
