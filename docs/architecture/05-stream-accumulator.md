...
2. **Struct lives in `StreamHandler`, not `Metrics`.** Defined as `ShhAi.ProviderClient.StreamHandler.Accumulator`. Mutated dominantly in `StreamHandler.handle_chunk/3`; read once at finalization by `Metrics.emit_stream_stop/3`.
...
5. **`Metrics.emit_stream_stop/3` takes typed arguments.** The accumulator is `%Accumulator{}`; per-request values are a typed spec from `StreamContext`. Field access is compile-time checked; no more `metrics_context.metrics_opts[:source_provider]` guessing.
...
|---|---|---|
| `start_time` | Per-request | `StreamContext` per-request half |
| `metrics_opts` | Per-request | `StreamContext` per-request half |
| `pii_info` | Per-request | `StreamContext` per-request half |
| `method` | Per-request | `StreamContext` per-request half |
| `source_path` | Per-request | `StreamContext` per-request half |
...
```
  ...
  └──────────┬──────────┘
             ...
             ▼
  ┌──────────────────────────────────┐
  │  Metrics.emit_stream_stop/3      │  ← read-side consumer
  │    accumulator :: %Accumulator{} │     typed contract at the seam
  │    per_request :: StreamContext  │     per-request half
  └──────────────────────────────────┘
...
- `lib/shh_ai/metrics.ex` — `emit_stream_stop/3` signature accepts `%StreamHandler.Accumulator{}` and per-request spec from `StreamContext`; 13 field accesses become compile-time-checked struct field reads.
...
- `emit_stream_stop/3` tests in `test/shh_ai/metrics_test.exs` construct a typed `%Accumulator{}` directly instead of building a map with the right keys. No more guessing whether `:restore_duration` or `:restore_time` is the correct key name—the compiler catches it.
